...
if (count($_POST) > 1) {
    ...
    $sql_stmt = "INSERT INTO `js_libraries` (`choice`,`ts`) VALUES ($option,NOW());";

    $model->insert($sql_stmt);
    
    $sql_stmt = "SELECT COUNT(choice) FROM js_libraries;";
    ...
    for ($i = 1; $i < 5; $i++) {
        ...
        $sql_stmt = "SELECT COUNT(choice) FROM js_libraries WHERE choice = $i;";
